...
// $Id: player_class.php 905 2007-02-28 21:33:05Z thindil $
    ...
    function logIn($strEmail, $pllimit) ///TODO: make it public static on php5 server
    {
        global $db;
        global $smarty;
        if (!$strEmail) {
        ...
        $strEmail = $db -> qstr($strEmail);
        $query = $db -> Execute('SELECT `id`, `user`, `email`, `rank`, `freeze`, `fight` FROM `players` WHERE `email`='.$strEmail.'') or die($db -> ErrorMsg());
        ...
        if ($query -> fields['rank'] != 'Admin' && $query -> fields['rank'] != 'Staff') {   // More players in game than it was intended.
...
class Player
{
    ...
    */
    public function __construct($strEmail, $strTitle)
