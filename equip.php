...
if (isset($_GET['schowaj'])) {
    if (!preg_match("/^[1-9][0-9]*$/", $_GET['schowaj'])) {
        error(ERROR);
    }
...
if (isset($_GET['sell_potion'])) {
    if (!preg_match("/^[1-9][0-9]*$/", $_GET['sell_potion'])) {
        error(ERROR);
    }
    ...
    if (preg_match("/(K)/", $sell -> fields['name']) || preg_match("/(S)/", $sell -> fields['name'])) {
...
*/
if (isset($_GET['sell'])) {
    if (!preg_match("/^[1-9][0-9]*$/", $_GET['sell'])) {
        error(ERROR);
    }
...
if (isset($_GET['sell_allpotion'])) {
    ...
    while (!$cost_potion -> EOF) {
        if (preg_match("/(K)/", $cost_potion -> fields['name']) || preg_match("/(S)/", $cost_potion -> fields['name'])) {
...
if (isset($_GET['napraw'])) {
    if (!preg_match("/^[1-9][0-9]*$/", $_GET['napraw'])) {
        error(ERROR);
    }
...
if (isset($_GET['poison'])) {
    if (!preg_match("/^[1-9][0-9]*$/", $_GET['poison'])) {
        error(ERROR);
    }
    ...
    if (isset($_GET['step']) && $_GET['step'] == 'poison') {
        if (!isset($_POST['weapon']) || !preg_match("/^[1-9][0-9]*$/", $_POST['weapon'])) {
            error(ERROR);
        }
        ...
        if (preg_match("/(K)/", $poison -> fields['name'])) {
            $intRoll = rand(0, 100);
            if ($intRoll == 1) {
        ...
        if ($intRoll > 50) {
            ...
            if (preg_match("/Dynallca/", $poison -> fields['name'])) {
                $strPtype = 'D';
            } elseif (preg_match("/Nutari/", $poison -> fields['name'])) {
                $strPtype = 'N';
            } elseif (preg_match("/Illani/", $poison -> fields['name'])) {
                $strPtype = 'I';
            }
...
*/
if (isset($_GET['fill'])) {
    if (!preg_match("/^[1-9][0-9]*$/", $_GET['fill'])) {
        error(ERROR);
    }
...
if (isset($_GET['wypijwiele'])) {
    ...
    if (!preg_match("/^[1-9][0-9]*$/", $_POST['ile'])) {
...
if ($mik -> fields['id']) {
    ...
    while (!$mik -> EOF) {
        ...
        if (preg_match("/(K)/", $mik -> fields['name']) || preg_match("/(S)/", $mik -> fields['name'])) {
