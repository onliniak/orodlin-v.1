...
if (isset($_GET['view']) && $_GET['view'] == 'immu') {
    ...
    $smarty -> assignByRef('Immuinfo', $immuInfo);
    $smarty -> assignByRef('Showoption', $showOption);
    $smarty -> assignByRef('Immunity', $player -> immunited);
...
if (isset($_GET['view']) && $_GET['view'] == "name") {
    ...
    if (isset($_GET['step']) && $_GET['step'] == "name") {
        ...
        if ($_POST['name'] == 'Admin' || $_POST['name'] == 'Staff' || empty($_POST['name']) || !preg_match("/[[:graph:]]/", $_POST['name'])) {
            error(ERROR);
        }
...
if (isset($_GET['view']) && $_GET['view'] == 'eci') {
    ...
    if (isset($_GET['step']) && $_GET['step'] == "gg") {
        ...
        if ($_POST['Change']) {
            if ($intKey === 0) {
                if (!preg_match("/^[1-9][0-9]*$/", $_POST['gg'])) {
                    error(ERROR);
                }
            }
...
if (isset($_GET['view']) && $_GET['view'] == 'style') {
    ...
    */
    ...
    $i = 0;
    while ($file = readdir($dir)) {
        if (!preg_match("/.htm*$/", $file) && !preg_match("/.tpl*$/", $file)) {
            if (!preg_match("/\..*$/", $file)) {
                $arrname1[$i] = $file;
                $i = $i + 1;
            }
