...
* Konstruktor
*/
    public function Huta()
    {
        global $player;
        if ((isset($_GET['step']) && !preg_match("/^[a-zA-Z0-9]/", $_GET['step'])) || (isset($_GET['action']) &&  !preg_match("/^[a-zA-Z0-9]/", $_GET['action']))) {
            error(ERROR);
        }
        if (!isset($_GET['step'])) {
            $this->step='';
...
*/
    public function sztabka($typ, $maxwt)
    {
        $sztabka='steel';
        if ($typ=='A' || $typ='W') {
...
*/
    public function showpoisoned($intPoisonPower, $strPoisonType)
    {
        $strPoisonInfo ='';
        if ($intPoisonPower > 0) {
 ...
 */
    public function backpack($playerid)
    {
        global $smarty;
        global $db;
 ...
 */
    public function przetop_wszystkie()
    {
        global $smarty;
        global $db;
        global $player;
 ...
 */
    public function przetop()
    {
        ...
        if (!empty($this->action) && $this->action>0) {
            ...
            if (!isset($amount) || $amount == '' || $amount <= 0) {
                error(ERROR);
            }
