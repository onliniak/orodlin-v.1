...
function mainreset()
{
    ...
    $db -> Execute('UPDATE `players` SET `age`=`age`+1, `houserest`=\'N\', `papurest`=\'N\'');
    ...
    if (!empty($arrTest)) {
        ...
        for ($i = 0; $i < 26; ++$i) {
           ...
           // echo('<p>'.$arrItems[$i].'. Cena wyliczana wzorkiem: '.$intPrice.'<br />');
            if ($arrMineral[0][3] < 1) {// ...find out how long the warehouse is empty, and increment price a bit more.
