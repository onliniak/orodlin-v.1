...
<div class="six columns">
	<a><img class="u-max-full-width" src="images/locations/temple.jpg" /></a>
	{if !in_array($smarty.get.view, array('book','pantheon','service'))}
	{$smarty.const.TEMPLE_INFO}
	{if $Location == 'Altara'}
	{$God2}. {$smarty.const.TEMPLE_INFO2}
	{/if}
	<ul {if $Graphstyle=="Y" }class="sword" {/if}> <li><a href="temple.php?view=service">{$smarty.const.A_WORK}</a></li>
	...
	<div id="prayer">
		<form method="post" action="temple.php?view=prayer">
			...
			<div class="columnright">{/if}
				<input id="bl{$smarty.section.temple.iteration}" type="radio" name="pray" value="{$Indices[temple]}" {if $smarty.section.temple.first} checked="checked" {/if} /><label for="bl{$smarty.section.temple.iteration}">{$Blessings[temple][0]}
					({$Blessings[temple][1]})</label><br />
				{/section}</div>
			<br />
	...
	{/if}
	<i>{$Booktext}</i><br />
	{if $Next < 2} <a href="temple.php?view=book&amp;book={$Next+1}">{$smarty.const.NEXT_PAGE}</a>
		{/if}
		{/if}
