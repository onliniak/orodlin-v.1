...
<div class="six columns">
	...
	{if $Graphstyle == "Y"}
	{if $Location == 'Altara'}
	<a><img class="u-max-full-width" id="meredithpicture" src="images/locations/meredith.jpg" alt="img_{$smarty.const.SHOW_IMAGE}" onclick="showHideImage('meredithpicture','images/locations/meredith.jpg')" /></a>
	{/if}
	{if $Location == 'Ardulith'}
	<a><img class="u-max-full-width" id="agarakarpicture" src="images/locations/agarakar.jpg" alt="img_{$smarty.const.SHOW_IMAGE}" onclick="showHideImage('agarakarpicture','images/locations/agarakar.jpg')" /></a>
	{/if}
	{/if}
	{$smarty.const.CITY_INFO}<br />
	<table width="100%">
		...
		<tr>
			...
			<td width="33%" valign="top">
				<div class="citytitle">{$Titles[row1]}</div>
				<ul{if $Graphstyle=="Y" } class="sword" {/if}> {section name=locations loop=$Files[row1]} <li><a href="{$Files[row1][locations]}">{$Names[row1][locations]}</a></li>
					{/section}
					</ul>
			</td>
	...
	<div class="center">
		...
		<b>{$News.tytul}</b>{$smarty.const.AUTOR}<b>{$News.autor}</b>{if $Rank == "Admin" || $Rank == "Staff"} <a href="city.php?step=ogloszenie&amp;del={$News.id}" onclick="return confirm('{$smarty.const.ERR_CONFIRM}');"
			title="{$smarty.const.ERR_DEL}">{if $Graphstyle == "Y"}<img src="images/icons/delete.png" alt="_img{$smarty.const.ERR_DEL}" />{else}{$smarty.const.ERR_DEL}{/if}</a>{/if}<br /><br />
		"{$News.tresc}"<br /><br />
	...
	<div class="center">
		...
		<br /><b>{$ArrNews[i][2]}</b>{$smarty.const.AUTOR}<b>{$ArrNews[i][1]}</b>{if $Rank == "Admin" || $Rank == "Staff"} <a href="city.php?step=list&amp;del={$ArrNews[i][0]}" onclick="return confirm('{$smarty.const.ERR_CONFIRM}');"
			title="{$smarty.const.ERR_DEL}">{if $Graphstyle == "Y"}<img src="images/icons/delete.png" alt="_img{$smarty.const.ERR_DEL}" />{else}{$smarty.const.ERR_DEL}{/if}</a>{/if}<br /><br />
		"{$ArrNews[i][3]}".<br><br>
		{/section}
