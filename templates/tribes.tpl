...
{if $View == "my"}
    ...
    {if $Step == ""}
        ...
        {$Welcome}<br/><br/>
        <table width="60%">
        <tr><td width="40%">{$Clanname}:</td><td width="60%"> {$Name}</td></tr>
        <tr><td>{$Memamount}:</td><td> {$Members}</td></tr>
        <tr><td>{$Leader}:</td><td> <a href="view.php?view={$Ownerid}">{$Owner}</a></td></tr>
        <tr><td>{$Goldcoins}:</td><td> {$Gold}</td></tr>
        <tr><td>{$Mithcoins}:</td><td> {$Mithril}</td></tr>
        <tr><td>{$Winamount}:</td><td> {$Wins}</td></tr>
        <tr><td>{$Lostamount}:</td><td> {$Lost}</td></tr>
        <tr><td>{$Tsoldiers}:</td><td> {$Soldiers}</td></tr>
        <tr><td>{$Tforts}:</td><td> {$Forts}</td></tr>
        <tr><td>{$WWW}</td></tr>
        <tr><td>{$Amachine}</td><td> ({$Percent}% {$Apercent})</td></tr>
        </table>
        {$Privmessage}
    {/if}
    {if $Step == "astral"}
