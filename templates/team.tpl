...
<div class="six columns" id="content">
	...
	{if $Admin == 1}
	<a href="team.php?mode=delete&amp;id={$Member.ID}{$AddrSufix}">{if $Graphstyle == "Y"}<img src="images/icons/delete.png" title="{$smarty.const.TEAM_DELETE}" alt="img_{$smarty.const.TEAM_DELETE}" />{else}{$smarty.const.TEAM_DELETE}{/if}</a> <a
		href="team.php?mode=modify&amp;id={$Member.ID}{$AddrSufix}">{if $Graphstyle == "Y"}<img src="images/icons/edit.png" title="img_{$smarty.const.TEAM_MODIFY}" alt="{$smarty.const.TEAM_MODIFY}" />{else}{$smarty.const.TEAM_MODIFY}{/if}</a>
	<br />
	{/if}
	...
	<ul{if $Graphstyle=="Y" } class="sword" {/if}> <li><a href="team.php?mode=delete&amp;step=write&amp;id={$ID}{$AddrSufix}">{$smarty.const.YES}</a></li>
