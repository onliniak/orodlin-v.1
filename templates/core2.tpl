...
{else}
	...
	{elseif $view == "search"}
		...
		{else}
			...
			<ul>{$List}</ul>
			{if $Gainexp > 0}
			{$smarty.const.YOU_GAIN} <b>{$Gainexp}</b> {$smarty.const.EXPERIENCE}
			{/if}
		{/if}
		<br/><br/>
		<a href="core.php">{$smarty.const.BACK}</a>
	{elseif $view == "market"}
		...
		{if $SellNamelist != ""}
			...
			{section name=foo loop=$SellNamelist}
				<option value="{$SellIdlist[foo]}">
					{$SellNamelist[foo]} :: {$SellAttacklist[foo]}, {$SellDefencelist[foo]}, {$SellSpeedlist[foo]} :: {$SellHplist[foo]}
				</option>
			{/section}
			</select>
			<br/>
		...
		{section name=foo loop=$Namelist}
			...
			<br/>
			{$smarty.const.C_ATTACK}: <b>{$Attacklist[foo]}</b>, {$smarty.const.C_DEFENCE}: <b>{$Defencelist[foo]}</b>, {$smarty.const.C_SPEED}: <b>{$Speedlist[foo]}</b>, {$smarty.const.C_HP}: <b>{$Hplist[foo]}</b>.
			</li>
		{/section}
		</ul>
		{/if}
	...
	{elseif $view == "monuments"}
		...
		<fieldset style="width: 70%; margin:auto;">
			...
			<table width="100%">
				...
				<tr bgcolor="{cycle values="#323232,#646464"}">
